...
internal sealed class WorldServiceTest
{
    ...
    public static void ClassInitialize(TestContext testContext)
    {
        ...
        {
            //Checking saving
            worldService.UpdateSerializer(ServerSerializers[index]);
            Assert.IsTrue(worldService.Save(worldData, tempSaveFilePath), $"Saving normal world failed while using {ServerSerializers[index]}.");
...
public sealed class DynamicWorldDataAfterAttribute : Attribute, ITestDataSource
{
    ...
    {
        return (WorldServiceTest.WorldsDataAfter ?? []).Select((t, i) => new object[] { t, WorldServiceTest.ServerSerializers[i].GetType().Name });
    }
